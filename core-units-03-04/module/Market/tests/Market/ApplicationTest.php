...
class MarketTest extends TestCase
{
        ...
        $sm = $this->serviceManager = new ServiceManager(
            new ServiceManagerConfig(array(
                ...
                'factories' => array(
                    ...
                    'Market'             => 'Zend\Mvc\Service\MarketFactory',
    ...
    public function testEventManagerListensOnMarketContext()
    {
        $events      = $this->application->getEventManager();
        $identifiers = $events->getIdentifiers();
        $expected    = array('Zend\Mvc\Market');
    ...
    public function testBootstrapRegistersConfiguredMvcEvent()
    {
        ...
        $this->assertSame($this->application, $event->getMarket());
    ...
    {
        $request = $this->serviceManager->get('Request');
        $uri     = UriFactory::factory($this->urlPath);
        $request->setUri($uri);
        ...
        if ($addService) {
            ...
            $controllerLoader->setFactory('path', 'Market\Factory\IndexControllerFactory');
        }
        $this->application->bootstrap();
    }
    ...
    public function setupActionController()
    {
        ...
        $controllerLoader = $this->serviceManager->get('ControllerLoader');
        $controllerLoader->setFactory('sample', 'Market\Factory\PostControllerFactory');
        $this->application->bootstrap();
    }
    ...
    public function setupBadController($addService = true)
    {
        ...
        if ($addService) {
            ...
            $controllerLoader->setFactory('bad', function () {
                return new ZendTest\Mvc\Controller\TestAsset\BadController;
            });
        }
        $this->application->bootstrap();
    }
    ...
    public function testControllerIsDispatchedDuringRun()
    {
        ...
        $this->assertContains('IndexController', $response->getContent());
    ...
    public function testInabilityToRetrieveControllerShouldTriggerExceptionError()
    {
        ...
        $this->application->run();
        $this->assertContains(Market::ERROR_CONTROLLER_NOT_FOUND, $response->getContent());
    ...
    public function testInvalidControllerTypeShouldTriggerDispatchError()
    {
        ...
        $this->application->run();
        $this->assertContains(Market::ERROR_CONTROLLER_INVALID, $response->getContent());
        $this->assertContains('bad', $response->getContent());
    }
    ...
    public function testRoutingFailureShouldTriggerDispatchError()
    {
        ...
        $this->assertContains(Market::ERROR_ROUTER_NO_MATCH, $response->getContent());
    }
    ...
    public function testFailureForRouteToReturnRouteMatchShouldPopulateEventError()
    {
        ...
        $this->application->run();
        $this->assertTrue($event->isError());
        $this->assertEquals(Market::ERROR_ROUTER_NO_MATCH, $event->getError());
    }
    ...
    public function testMarketShouldBeEventTargetAtFinishEvent()
    {
        $this->setupActionController();
        ...
        $this->application->run();
        $this->assertContains('Zend\Mvc\Market', $response->getContent());
    }
